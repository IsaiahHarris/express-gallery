        ...
        if (!arts) {
          ...
          let artsArr = arts.map(element => {
            element.attributes.username =
              element.relations.author.attributes.username;
...
router
  ...
  .put((req, res) => {
    ...
    return Art.query({ where: { id: id } })
      ...
      .then(result => {
        if (
          req.user.id !== result.models[0].attributes.author_id &&
          req.user.id !== 25
        ) {
          req.flash('youDontOwn', 'You do not have rights to edit this post');
          return res.redirect(`/arts/${id}`);
        } else {
          return new Art({ id: id }).save({ author_id, link, description });
        }
      })
      .then(art => {
        if (!art) {
